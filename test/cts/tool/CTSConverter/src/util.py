...
OperationsInfoDict = {
    ...
    'AVERAGE_POOL_2D': {
        'equalOp': 'averagePool2d',
        # ['input', 'padding left', 'padding right', 'padding top', 'padding bottom', 'stride width', 'stride height', 'filter width', 'filter height', 'activation']
        'paramList': ['input', 'padding', 'padding', 'padding', 'padding', 'stride', 'stride', 'filter', 'filter', 'activation'] # current only for explicit paddings
    },
    'CONV_2D': {
        'equalOp': 'conv2d',
...
def ExcludeVarDefine(androidNNOpType, index):
    return OperationsInfoDict[androidNNOpType]['paramList'][index] in ['multiplier', 'activation']


def ConvertDimensions(dimensions, flag = False, groups = 1, layout = 'nhwc'):
    if layout == 'nhwc':
...
def GetOperandDesc(t, filter, depthwiseConv2dFlag, layout = 'nhwc'):
    dimensions = t.GetDimensionsString()[1:-1].split(',')
    dimensions = [d.strip() for d in dimensions]

    if filter and layout == 'nhwc':
        dimensions = ConvertDimensions(dimensions, depthwiseConv2dFlag)
...
def reorderValues(values, t, flag):
    dimensions = t.GetDimensionsString()[1:-1].split(',')
    dimensions = [int(i) for i in dimensions]
    valuesArray = np.array(values).reshape(dimensions)
    axes = (1, 2, 3, 0)
    if flag:
        axes = (1, 2, 0, 3)
    return list(np.transpose(valuesArray, axes).ravel())
